...
public class AdminProductDaoImpl {
    private static final Logger logger = LogManager.getLogger(AdminProductDaoImpl.class);
    ...
    public Integer saveProduct(ProductForm form) throws Exception {
        logger.info("Saving admin-managed product {}", form.getProductName());
        ...
        try {
            ...
            sess.rollback();
            logger.error("Failed to save admin product {}: {}", form.getProductName(), ex.getMessage(), ex);
            throw ex;
        } finally {
            sess.close();
        }
    ...
    public void closeProduct(Integer productId) throws Exception {
        logger.info("Closing product {}", productId);
