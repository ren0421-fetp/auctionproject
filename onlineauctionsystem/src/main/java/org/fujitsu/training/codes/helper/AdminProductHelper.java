...
public class AdminProductHelper {
	...
	public AdminProductHelper(AdminProductDaoImpl adminProductDaoImpl, FileService fileService) {
		this.adminProductDaoImpl = adminProductDaoImpl;
		this.fileService = fileService;
	}

	public void prepareLoadProducts(Integer productId, String status, String sellerUsername, String success,
			Model model) {
		logger.info("Loading admin product page. productId={}, status={}, sellerUsername={}", productId, status,
				sellerUsername);
	...
	public String processSaveProduct(ProductForm form, BindingResult result, Model model) {
		logger.info("Processing admin product save. productId={}, productName={}", form.getProductId(),
				form.getProductName());
		...
		if (result.hasErrors()) {
			...
			logger.warn("Admin product validation failed. productId={}, productName={}", form.getProductId(),
					form.getProductName());
			return VIEW_NAME;
		}
		...
		try {
			...
			logger.info("Admin product save completed. productId={}, productName={}", form.getProductId(),
					form.getProductName());
