...
public class PackageFormValidator implements Validator {
	...
	public void validate(Object target, Errors errors) {
		...
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "packageName", "package_name.required",
				"Package name is required.");

		if (form.getPackageName() != null && form.getPackageName().trim().length() > 100) {
			errors.rejectValue("packageName", "package_name.length", "Package name must not exceed 100 characters.");
		}

		if (form.getPackagePrice() == null) {
			errors.rejectValue("packagePrice", "package_price.required", "Package price is required.");
		} else if (form.getPackagePrice().compareTo(BigDecimal.ZERO) < 0) {
			errors.rejectValue("packagePrice", "package_price.invalid", "Package price must not be negative.");
		}

		if (form.getAllowedBidCount() == null) {
			errors.rejectValue("allowedBidCount", "allowed_bid_count.required", "Allowed bid count is required.");
