...
public class BidHelper {
	...
	public void prepareOpenProducts(String keyword, Integer catId, BigDecimal minPrice, BigDecimal maxPrice,
			Model model) {
		logger.info("Loading bidder auction list. keyword={}, catId={}, minPrice={}, maxPrice={}", keyword, catId,
				minPrice, maxPrice);

		model.addAttribute("openProducts", bidDaoImpl.getOpenProducts(keyword, catId, minPrice, maxPrice));
	...
	public String prepareProductDetail(Integer productId, String success, String bidderUsername, Model model) {
		logger.info("Loading bidder product detail. bidderUsername={}, productId={}", bidderUsername, productId);
		...
		logger.info("Bidder product detail loaded. bidderUsername={}, productId={}", bidderUsername, productId);
		return DETAIL_VIEW;
	}

	public String processSubmitBid(BidForm bidForm, BindingResult result, Model model, String bidderUsername) {
		logger.info("Processing bid submission. bidderUsername={}, productId={}", bidderUsername,
				bidForm.getProductId());

		Product product = bidDaoImpl.getProductDetail(bidForm.getProductId());
		if (product == null) {
			logger.warn("Bid submission failed because productId={} was not found.", bidForm.getProductId());
			return LIST_REDIRECT;
		}

		if (result.hasErrors()) {
			populateDetailPage(model, product, bidForm, bidderUsername);
			logger.warn("Bid validation failed. bidderUsername={}, productId={}", bidderUsername,
					bidForm.getProductId());
			return DETAIL_VIEW;
		}
		...
		try {
			...
			logger.info("Bid submission completed. bidderUsername={}, productId={}", bidderUsername,
					bidForm.getProductId());
	...
	public String prepareModifyBidForm(Integer bidId, String bidderUsername, Model model) {
		...
		if (existingBid == null) {
			logger.warn("Modify-bid form load failed. bidId={} not found for bidder={}", bidId, bidderUsername);
			return MY_BIDS_REDIRECT;
		}

		Product product = bidDaoImpl.getProductDetail(existingBid.getProductId());
		if (product == null) {
			logger.warn("Modify-bid form load failed. productId={} not found for bidId={}", existingBid.getProductId(),
					bidId);
			return MY_BIDS_REDIRECT;
		}
	...
	public String processModifyBid(BidForm bidForm, BindingResult result, Model model, String bidderUsername) {
		logger.info("Processing bid modification. bidderUsername={}, bidId={}", bidderUsername, bidForm.getBidId());

		Bid existingBid = bidDaoImpl.getBidderBid(bidForm.getBidId(), bidderUsername);
		if (existingBid == null) {
			logger.warn("Bid modification failed. bidId={} not found for bidder={}", bidForm.getBidId(),
					bidderUsername);
			return MY_BIDS_REDIRECT;
		}
		...
		if (result.hasErrors()) {
			...
			logger.warn("Bid modification validation failed. bidderUsername={}, bidId={}", bidderUsername,
					bidForm.getBidId());
			return MODIFY_VIEW;
		}
		...
		try {
			...
			logger.info("Bid modification completed. bidderUsername={}, bidId={}", bidderUsername, bidForm.getBidId());
			return MY_BIDS_REDIRECT;
		} catch (Exception ex) {
	...
	private void populateDetailPage(Model model, Product product, BidForm bidForm, String bidderUsername) {
		model.addAttribute("product", product);
		model.addAttribute("bidForm", bidForm);
		model.addAttribute("remainingBidCount", bidDaoImpl.getRemainingBidCount(bidderUsername));
	}

	private void populateModifyPage(Model model, Product product, Bid existingBid, BidForm bidForm,
