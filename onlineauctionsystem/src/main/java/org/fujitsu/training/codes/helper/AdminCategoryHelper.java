...
public class AdminCategoryHelper {
	...
	public String processSaveCategory(CategoryForm form, BindingResult result, Model model) {
		logger.info("Processing category save. catId={}, categoryName={}", form.getCatId(), form.getCatName());

		if (result.hasErrors()) {
			populatePage(model, form);
			logger.warn("Category validation failed. catId={}, categoryName={}", form.getCatId(), form.getCatName());
			return VIEW_NAME;
		}

		try {
			adminCategoryDaoImpl.saveCategory(form);
			logger.info("Category save completed. catId={}, categoryName={}", form.getCatId(), form.getCatName());
