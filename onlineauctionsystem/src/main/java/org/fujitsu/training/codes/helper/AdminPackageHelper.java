...
public class AdminPackageHelper {
	...
	public AdminPackageHelper(AdminPackageDaoImpl adminPackageDaoImpl, FileService fileService) {
	...
	public String processSavePackage(PackageForm form, BindingResult result, Model model) {
		logger.info("Processing admin package save. packageId={}, packageName={}", form.getPackageId(),
				form.getPackageName());

		if (result.hasErrors()) {
			populatePage(model, form);
			logger.warn("Admin package validation failed. packageId={}, packageName={}", form.getPackageId(),
					form.getPackageName());
			return VIEW_NAME;
		}
		...
		try {
			...
			logger.info("Admin package save completed. packageId={}, packageName={}", form.getPackageId(),
					form.getPackageName());
