...
public class SellerProductHelper {
	...
	public SellerProductHelper(SellerProductDaoImpl sellerProductDaoImpl, FileService fileService) {
	...
	public String prepareEditForm(Integer productId, String sellerUsername, Model model) {
		...
		if (product == null) {
			logger.warn("Seller edit-product load failed. productId={} not found for seller={}.", productId,
					sellerUsername);
	...
	public String processSaveProduct(ProductForm form, BindingResult result, Model model, String sellerUsername) {
		...
		if (result.hasErrors()) {
			...
			logger.warn("Seller product validation failed for seller={} productId={}.", sellerUsername,
					form.getProductId());
			return FORM_VIEW;
		}
	...
	public String processDeleteProduct(Integer productId, Model model, String sellerUsername) {
		...
		try {
			...
			logger.error("Failed to delete product {} for seller {}: {}", productId, sellerUsername, ex.getMessage(),
					ex);
