...
public class AdminLocationHelper {
	...
	public void prepareLoadPage(Integer countryId, Integer stateId, Integer cityId, String success, Model model) {
		logger.info("Loading admin location page. countryId={}, stateId={}, cityId={}", countryId, stateId, cityId);
	...
	public String processSaveCountry(CountryForm form, BindingResult result, Model model) {
		logger.info("Processing country save. countryId={}, countryName={}", form.getCountryId(),
				form.getCountryName());

		if (result.hasErrors()) {
			populatePage(model, form, new StateForm(), new CityForm());
			logger.warn("Country validation failed. countryId={}, countryName={}", form.getCountryId(),
					form.getCountryName());
			return VIEW_NAME;
		}
		...
		try {
			...
			logger.info("Country save completed. countryId={}, countryName={}", form.getCountryId(),
					form.getCountryName());
	...
	public String processSaveState(StateForm form, BindingResult result, Model model) {
		logger.info("Processing state save. stateId={}, stateName={}", form.getStateId(), form.getStateName());

		if (result.hasErrors()) {
			populatePage(model, new CountryForm(), form, new CityForm());
			logger.warn("State validation failed. stateId={}, stateName={}", form.getStateId(), form.getStateName());
			return VIEW_NAME;
		}

		try {
			adminLocationDaoImpl.saveState(form);
			logger.info("State save completed. stateId={}, stateName={}", form.getStateId(), form.getStateName());
			return SUCCESS_REDIRECT;
		} catch (Exception ex) {
	...
	public String processSaveCity(CityForm form, BindingResult result, Model model) {
		logger.info("Processing city save. cityId={}, cityName={}", form.getCityId(), form.getCityName());

		if (result.hasErrors()) {
			populatePage(model, new CountryForm(), new StateForm(), form);
			logger.warn("City validation failed. cityId={}, cityName={}", form.getCityId(), form.getCityName());
			return VIEW_NAME;
		}

		try {
			adminLocationDaoImpl.saveCity(form);
			logger.info("City save completed. cityId={}, cityName={}", form.getCityId(), form.getCityName());
			return SUCCESS_REDIRECT;
		} catch (Exception ex) {
