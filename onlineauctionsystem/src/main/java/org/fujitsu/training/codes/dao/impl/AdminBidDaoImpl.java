...
public class AdminBidDaoImpl {
    private static final Logger logger = LogManager.getLogger(AdminBidDaoImpl.class);
    ...
    public void confirmWinningBid(Integer bidId) throws Exception {
        logger.info("Attempting to confirm winning bid {}", bidId);
        ...
        try {
            ...
            Integer existingCount = sess.selectOne(
                    "org.fujitsu.training.codes.dao.BidConfirmDao.countConfirmedByProductId",
                    bid.getProductId());
            ...
            Integer inserted = sess.insert(
                    "org.fujitsu.training.codes.dao.BidConfirmDao.insertBidConfirmation",
                    confirm);
            ...
            Integer closed = sess.update(
                    "org.fujitsu.training.codes.dao.ProductDao.closeProductById",
                    bid.getProductId());
            ...
            logger.info("Winning bid {} confirmed successfully", bidId);
        } catch (Exception ex) {
            sess.rollback();
            logger.error("Failed to confirm winning bid {}: {}", bidId, ex.getMessage(), ex);
            throw ex;
        } finally {
            sess.close();
        }
